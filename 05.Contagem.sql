...
begin   
	...
	select count(id)  INTO QTDE_DFE_LOB_LIMPEZA from msaf_dfe.dfe_lob where id < (select ID_LOB from msaf_dfe.limpeza_lobs_max_id where DT_CRIACAO IN (SELECT MAX(DT_CRIACAO) from msaf_dfe.limpeza_lobs_max_id)) and id not in (select distinct(id_lob) from msaf_dfe.limpeza_lobs);
